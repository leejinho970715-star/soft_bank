...
const regenerated=JSON.parse(await fs.readFile('assets/subpages/regenerated/manifest.json','utf8'));
const screenAsset=name=>regenerated[`assets/subpages/ifrs18/${name}.png`]||{...screenSizes.get(name),file:`assets/subpages/ifrs18/${name}.png`};
const picture=(name,alt)=>{const a=screenAsset(name);return `<picture><source type="image/webp" srcset="../../${a.file.replace(/\.png$/,'.webp')}"><img src="../../${a.file}" alt="${alt.replace('실제 서비스','서비스')}" loading="lazy" width="${a.width}" height="${a.height}"></picture>`};
const features=[
...
<section class="ifrs-intro">
 ...
 <div class="ifrs-actions"><a class="cta dark" href="../purchase/inquiry.html">도입 상담하기</a><a class="cta ifrs-outline" href="../../assets/subpages/ifrs18/introduction-poster.png" data-image-modal="poster" data-image-caption="Amaranth 10 IFRS18 소개 포스터" aria-haspopup="dialog">소개 자료 보기</a></div>
</section>
...
<section class="ifrs-features"><span class="ifrs-eyebrow">CORE FEATURES</span><h2>실무를 위한 세 가지 핵심 기능</h2>
 ${features.map(([n,img,label,title,copy,tags])=>`<article class="ifrs-feature"><span class="ifrs-eyebrow">${n} / ${label}</span><h3>${title}</h3><p>${copy}</p><div class="ifrs-tags">${tags.split(' · ').map(t=>`<span>${t}</span>`).join('')}</div><a class="ifrs-screen" href="../../${screenAsset(img).file}" aria-label="${label} 화면 크게 보기">${picture(img,`Amaranth 10 ${label} 실제 서비스 화면`)}</a><small>제품 기능 화면 · 선택하면 크게 볼 수 있습니다.</small></article>`).join('')}
</section>
