(() => {
  ...
  const close=()=>{try{sessionStorage.setItem('softbank-intro-seen','1')}catch{}overlay.remove();root.inert=false;document.body.classList.remove('intro-open');document.removeEventListener('keydown',key);root.querySelector('.brand')?.focus({preventScroll:true});};
